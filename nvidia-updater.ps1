<#
    ...
    1. Run the following command to create a Windows Schedule Task that runs the script on every logon. Change the location in the first line to where this script will run from.
        ...
        $SchTaskAction = New-ScheduledTaskAction -Execute "Powershell.exe" -Argument "-ExecutionPolicy Bypass d:\github\scripts\nvidia-updater.ps1"
...
$driverGracePeriod = 0                                      # How many days grace period before installing the new driver
...
function CompareDriverVersions {
    try {
        ...
        $installed_version = (Get-WmiObject Win32_PnPSignedDriver | Where-Object { $_.devicename -like "*nvidia*" -and $_.devicename -notlike "*audio*" -and $_.devicename -notlike "*USB*" -and $_.devicename -notlike "*SHIELD*" }).DriverVersion.SubString(7).Remove(1, 1).Insert(3, ".")
...
function GetDriver ($version) {
    ...
    try {
        ...
        $url = "https://de.download.nvidia.com/Windows/$version/$version-desktop-win10-64bit-international-whql.exe"
        ...
        $args = "x -bso0 -bsp1 $dlFile Display.Driver Display.Optimus PhysX NVI2 GFExperience EULA.txt ListDevices.txt setup.cfg setup.exe -o$extractDir"
